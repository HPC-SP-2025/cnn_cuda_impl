...
private:
    ...
    __global__ void forward_kernel(vector<float> &output, vector<float> &target, float *loss, int n, int num_classes)
    {
        int idx = blockDim.x * blockIdx.x + threadIdx.x;
        if (idx < n)
        {
            float loss_i = log(output[idx * num_classes + target[idx]] + EPSILON);
    ...
    __global__ void backward_kernel(vector<float> &grad_output, vector<float> &output, vector<float> &target, int n, int num_classes)
    {
        int idx = blockDim.x * blockIdx.x + threadIdx.x;
        if (idx < n)
        {
            ...
            grad_output[idx * num_classes + target[idx]] -= 1.0 / n;
        }
    }
};
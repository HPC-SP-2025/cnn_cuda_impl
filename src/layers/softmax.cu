...
__global__ void forwardKernelSoftmax(float *input, float *output, size_t num_classes, size_t batch_size) {
    ...
    if (idx < batch_size) {
        ...
        for (size_t i = 1; i < num_classes; ++i) {
            max_val = max(max_val, current_input[i]);
        }
...
float *Softmax::backward(float *grad_input) {
    if (!device) {
        ...
        if (test_mode) {
            cudaMemcpy(this->host_backward_buffer, this->device_backward_buffer,
                       sizeof(float) * output_size * batch_size, cudaMemcpyDeviceToHost);
            return this->host_backward_buffer;
        } else {
...
void Softmax::setDevice(int device) {
    ...
    if (device) {
        ...
        cudaMemcpy(this->device_forward_buffer, this->host_forward_buffer,
                   sizeof(float) * this->output_size * this->batch_size, cudaMemcpyHostToDevice);
        cudaMemcpy(this->device_backward_buffer, this->host_backward_buffer,
                   sizeof(float) * this->input_size * this->batch_size, cudaMemcpyHostToDevice);
    }
}
